...
describe("MessagingManager.handleMessage", () => {
  ...
  beforeEach(() => {
    ...
    (manager as any).mainChannelId = "main-channel-id";
    cmdAskSpy = spyOn(manager as any, "cmdAsk").mockResolvedValue(undefined);
  });
  ...
  it("does not route !-prefixed message in main channel to cmdAsk", async () => {
    const handleCommandSpy = spyOn(manager as any, "handleCommand").mockResolvedValue(undefined);
    const msg = {
      channelId: "main-channel-id",
